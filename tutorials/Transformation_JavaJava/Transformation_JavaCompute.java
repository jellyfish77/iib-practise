...
public class Transformation_JavaCompute extends MbJavaComputeNode {
	...
	public void evaluate(MbMessageAssembly inAssembly) throws MbException {
		...
		MbMessage outMessage = new MbMessage(); // create an empty output
												// message
		...
		MbElement outBody = outRoot.createElementAsLastChild("XMLNSC"); // create
																		// the
																		// 'Body'
																		// XMLNSC
																		// element

		// set up the iterative operations using ForEachChildOperation helper
		// class

		// declare the class to create articles for each invoice
		final ForEachChildOperation createArticle = new ForEachChildOperation(
				"Item") {
			...
			protected void forEachElement(MbElement item) throws MbException {
				MbElement purchases = getOutputElement(); // build the output
															// tree from this
															// point
				MbElement article = purchases.createElementAsLastChild(
						MbXMLNSC.FOLDER, "Article", null);
				MbElement cursor = item.getFirstElementByPath("Description");
				article.createElementAsLastChild(MbXMLNSC.FIELD, "Desc",
						(String) cursor.getValue());

				cursor = cursor.getNextSibling().getNextSibling(); // Price
																	// element
				...
				article.createElementAsLastChild(MbXMLNSC.FIELD, "Cost",
						Double.toString(cost));
				article.createElementAsLastChild(MbXMLNSC.FIELD, "Qty",
						quantity);
			}

			protected void after() throws MbException {
				total = (int) (total * 1000) / 1000.0; // rounding
				getOutputElement().createElementAfter(MbXMLNSC.FIELD, "Amount",
						Double.toString(total)).createElementAsFirstChild(
						MbXMLNSC.ATTRIBUTE, "Currency", curency);
			}
		};

		// declare the class to create statements for each salelist
		final ForEachChildOperation createStatement = new ForEachChildOperation(
				"Invoice") {
			protected void forEachElement(MbElement invoice) throws MbException {
				...
				String balance = (String) invoice.getFirstElementByPath(
						"Balance").getValue();
				curency = (String) invoice.getFirstElementByPath("Currency")
						.getValue();

				MbElement statement = outSaleList.createElementAsLastChild(
						MbXMLNSC.FOLDER, "Statement", null);
				statement.createElementAsLastChild(MbXMLNSC.ATTRIBUTE, "Type",
						"Monthly");
				statement.createElementAsLastChild(MbXMLNSC.ATTRIBUTE, "Style",
						"Full");

				MbElement customer = statement.createElementAsLastChild(
						MbXMLNSC.FOLDER, "Customer", null);
				customer.createElementAsLastChild(MbXMLNSC.FIELD, "Initials",
						initials);
				customer.createElementAsLastChild(MbXMLNSC.FIELD, "Name",
						surname);
				customer.createElementAsLastChild(MbXMLNSC.FIELD, "Balance",
						balance);

				MbElement purchases = statement.createElementAsLastChild(
						MbXMLNSC.FOLDER, "Purchases", null);
		...
		// declare the class to create the output salelist for each input
		// salelist
		final ForEachChildOperation createSaleList = new ForEachChildOperation(
				"SaleList") {
			protected void forEachElement(MbElement saleList)
					throws MbException {

				MbElement outSaleList = getOutputElement()
						.createElementAsLastChild(MbXMLNSC.FOLDER, "SaleList",
								null);
		...
		MbElement outSaleEnvelope = outBody.createElementAsFirstChild(
				MbXMLNSC.FOLDER, "SaleEnvelope", null);
